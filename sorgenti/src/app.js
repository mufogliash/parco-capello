...
const etichettaFoto = (im) => voc('tipo_immagine', im.tipo) + (im.stagione ? ' in ' + im.stagione : '');
...
V.scheda = ([id, sez = 'riconosci']) => {
  ...
  const gal = ims.length ? `<div class="gallery" data-gallery>${ims.map((im, i) => `<figure><img src="${esc(imgSrc(im.file))}" alt="${esc(s.nome_comune + ': ' + etichettaFoto(im))}" data-view="${i}" ${i ? 'loading="lazy"' : ''}><figcaption>${esc(etichettaFoto(im))} · ${esc(im.autore)} · ${esc(im.licenza)}</figcaption></figure>`).join('')}</div>
...
V.info = () => {
  ...
  <div class="pad stack" style="gap:22px">
    ...
    <section class="stack" style="gap:8px"><h3>Crediti delle immagini</h3>
      ${crediti.length ? `<ul style="margin:0;padding-left:18px;font-size:14px;display:flex;flex-direction:column;gap:6px">${crediti.map(({ s, im }) => `<li>${esc(s.nome_comune)} (${esc(voc('tipo_immagine', im.tipo))}): ${esc(im.autore)}, ${im.url_originale ? `<a href="${esc(im.url_originale)}" target="_blank" rel="noopener">${esc(im.licenza)}</a>` : esc(im.licenza)}, ${esc(im.fonte)}</li>`).join('')}</ul>` : '<p class="muted">Nessuna immagine ancora.</p>'}
...
function apriVisore(im) {
  ...
  v.innerHTML = `<img src="${esc(imgSrc(im.file))}" alt=""><div class="cap">${esc(im.autore)} · ${esc(im.licenza)} · ${im.url_originale ? `<a href="${esc(im.url_originale)}" target="_blank" rel="noopener">${esc(im.fonte)}</a>` : esc(im.fonte)}</div><button class="icon-btn" aria-label="Chiudi">${ico('x')}</button>`;
